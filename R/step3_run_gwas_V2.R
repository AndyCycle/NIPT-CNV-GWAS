...
run_nipt_gwas <- function(int_mat_file, sample_list_file,
  ...
  .align_meta <- function(infile, ids, target_col = NULL, type_name = "Meta") {
    ...
    report_mem(sprintf("Step3: Aligning %s - Start", type_name))

    # 1. 快速读取 (只读必要列)
    # 先读一行表头定列名
    header_preview <- data.table::fread(infile, header = FALSE, nrows = 1)
    col_names <- paste0("V", 1:ncol(header_preview))
    col_names[1:2] <- c("FID", "IID")

    # 确定需要哪些列
    if (is.null(target_col)) {
      # 全读 (Covariates)
      dt <- data.table::fread(infile, header = FALSE, col.names = col_names)
      vars_to_keep <- setdiff(names(dt), c("FID", "IID"))
    } else {
      # 只读特定列 (Phenotype)，利用 select 参数极大降低内存
      # 需要先确定 target_col 是第几列
      # 这里假设用户知道或者文件比较规范。如果比较复杂，先全读进来也行(表型文件通常小)
      dt <- data.table::fread(infile, header = FALSE, col.names = col_names)
